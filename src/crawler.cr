struct Crawler
  ...
  def self.validate_csv(file, fmt)
    ...
    cardlist.each do |entry|
      ...
      Log.info { "#{fmt}: #{legalities} #{card_name} ◄ #{set_name} ► ⑇ #{quantity}" }
    end
  end
